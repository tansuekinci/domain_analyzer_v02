...
def write_file(filename, subdomains):
    ...
    try:
        ...
        print("%s[+] Dosya başarıyla oluşturuldu: %s%s" % (G, filename, W)) # Başarı mesajı
    except Exception as e:
        print("%s[!] Hata: Dosya oluşturulamadı veya yazılamadı. Hata: %s%s" % (R, e, W))
...
# Yeni Hali (Düzeltilmiş Kod)
    def get_csrftoken(self, resp):
        csrf_regex = re.compile('<input type="hidden" name="csrfmiddlewaretoken" value="(.*?)">', re.S)
        tokens = csrf_regex.findall(resp)
        if not tokens:
            self.print_(R + "[!] Hata: DNSdumpster CSRF belirteci bulunamadı. Erişim engellenmiş olabilir." + W)
            return None
    ...
    def enumerate(self):
        ...
        # Eğer token alınamazsa (None dönerse), işlemi sonlandır.
        if token is None:
            return self.subdomains
...
class PassiveDNS(enumratorBaseThreaded):
    ...
    def extract_domains(self, resp):
        try:
            ...
            print(Y + "[-] PassiveDNS API'den gelen sonuç sayısı: %s" % len(subdomains) + W)
            for subdomain in subdomains:
                if subdomain not in self.subdomains and subdomain != self.domain:
                    if self.verbose:
                        self.print_("%s%s: %s%s" % (R, self.engine_name, W, subdomain))
